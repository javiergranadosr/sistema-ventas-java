...
public class ClientController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        switch (view) {
            case "clients":
                request.setAttribute("title", "Sistema ventas | Clientes");
                request.setAttribute("active_client", "active");
    ...
    /**
     * Acciones a realizar desde la pagina cliente
     *
     * @param action
     */
    private void action(String action, HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        switch (action) {
            case "read":
                List<User> clients = new UserDAO(null).fetchAll("Cliente");
            ...
            case "add":
                ...
                String clientSurname = request.getParameter("client_surname");
                String clientEmail = request.getParameter("client_email");
                String clientPassword = request.getParameter("client_password");
                String clientState = request.getParameter("client_state");

                UserDAO create = new UserDAO(conn);
                User userCreate = new User(clientDocument, clientName,clientSurname, clientEmail, clientPassword, "Cliente", clientState);
                create.create(userCreate);
                conn.commit();
                request.getRequestDispatcher("client?view=clients&action=read").forward(request, response);
            ...
            case "update":
                ...
                User userUpdate = new User(userId,clientDocument, clientName,clientSurname, clientEmail, clientPassword, "Cliente", clientState);
